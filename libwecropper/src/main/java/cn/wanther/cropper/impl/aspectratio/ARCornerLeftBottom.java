 ...
 */
public class ARCornerLeftBottom extends CornerLeftBottom {
    public ARCornerLeftBottom(Engine engine) {
        super(engine);
    }
    ...
    public boolean isTouched(float x, float y) {
        ...
        return x >= getX() - getTouchExtension() && x <= getX() + cropWidthHalf
                && y >= getY() - getTouchExtension() && y <= getY() + getTouchExtension()
                ||
                x >= getX() - getTouchExtension() && x <= getX() + getTouchExtension()
                && y >= getY() - cropHeightHalf && y <= getY() + getTouchExtension();
    }
